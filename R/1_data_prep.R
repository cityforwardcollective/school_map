...
mps <- c("Traditional Public", "Instrumentality Charter", "Partnership")

charters <- c("Non-Instrumentality Charter", "2r/2x Charter")
...
mke_schools <- schools %>%
  ...
  mutate(
    accurate_agency_type = case_when(
      accurate_agency_type %in% mps ~ "Traditional Public Schools",
...
mke_tested_students_aat <- act %>%
  ...
  right_join(., schools) %>%
  filter(
    !accurate_agency_type %in% c("Private Choice Schools", "Partnership")
  ) %>%
  mutate(
    pa = case_when(
      test_result %in%
        c("Below Basic", "Basic", "Developing", "Approaching") ~ "bb",
      test_result %in% c("Proficient", "Advanced", "Meeting") ~ "pa",
...
private <- schools %>%
  filter(
    accurate_agency_type == "Private Choice Schools" &
      school_year == sy
  ) %>%
...
aspire <- forward_exam |>
  filter(
    group_by_value == "All Students" &
      ((test_group %in% c("Aspire", "DLM") & grade %in% c("9", "10")) |
        test_group == "PreACT") &
      school_year == sy
  ) %>%
  right_join(., schools) %>%
...
hs |>
  ...
  left_join(
    schools |>
      filter(accurate_agency_type != "Partnership") |>
      select(dpi_true_id, school_year, broad_agency_type)
  ) |>
...
xx <- all_prof |>
  ...
  right_join(
    make_wi_rc(private_type = "all", exclude_milwaukee = FALSE) |>
  ...
  mutate(
    accurate_agency_type = case_when(
      accurate_agency_type %in% mps ~ "Traditional Public School",
  ...
  left_join(
    schools |>
      select(dpi_true_id, school_year, milwaukee_indicator)
  )

cfc_scored <- map2_df(
  c("sch_ach", "sch_growth", "sch_tgo", "sch_ot"),
  c("ach_weight", "growth_weight", "tgo_weight", "ot_weight"),
  function(x, y) {
    make_wi_rc(exclude_milwaukee = FALSE) |>
      ungroup() |>
      filter(school_year == sy2) |>
      select(school_year, dpi_true_id, value = x, weight = y) |>
      mutate(score = x)
  ...
  mutate(
    half_weight = sum(weight, na.rm = TRUE) / 2,
  ...
  summarise(
    cfc_score = weighted.mean(value, new_weight, na.rm = TRUE) |>
      round(1)
  ) |>
  ungroup() |>
  mutate(
...
rc_links <- read_csv("../wi_report_cards/data/school_id_table.csv")
...
xx <- left_join(
  ...
  rc_links |>
    transmute(
      ...
      dpi_true_id,
      rc_link = case_when(
        rc_n == "choice_only" ~ `Choice Students`,
        rc_n == "both" ~ `All Students`,
        rc_n == "all_only" ~ `All Students`,
        TRUE ~ ""
      )
    )
)

saveRDS(xx, "data/internal/prepped_data_1.rda")
